...
>    * 陣列屬於一種資料結構，主要是用一段連續記憶體空間來儲存一組類似的資訊，因此具有結構性，也就比較容易進行存取的動作
>    * 宣告時與一般變數類似，但須在變數名稱後方加上中括號，賦予值的時候需要加上大括號，Ex: `int arr[2] = {0, 1};`
>    * 若陣列宣告時，所賦予的值不滿足元素個數，其餘空間會自動補上 0
>    * 若陣列宣告時，也可不定義元素個數，那就以後方賦予值的資料長度來定義，Ex: `int arr[] = {0, 1, 2, 3}`，系統會自動定義元素個數為 4
>    * 陣列變數名稱即為該陣列的起始記憶體位置，若需傳遞陣列進入函式，只要傳陣列變數名稱即可
>    * 若傳遞陣列進入函式，通常連元素個數一起傳入，方便存取時使用
>    * 一般變數傳入另一個函示會複製一份資料，陣列僅複製要傳遞的開頭指標，並將此指標傳入其他函式，所以若在其他函式內改變陣列內儲存的值，都會直接連動到原本陣列，使用時要謹慎思考這個特性
